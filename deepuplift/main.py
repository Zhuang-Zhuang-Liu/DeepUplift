...
if __name__ == "__main__":
  ...
  est3 = DragonNet(input_dim=len(features),share_dim=64, task='regression' )
  est4 = DragonDeepFM(input_dim=12,num_continuous=12,share_dim=32,embedding_size=8,
                      task='classification', num_treatments=2)
  est5 = EFIN(input_dim=12, hc_dim=16, hu_dim=8, is_self=True, act_type="elu") 
  est6 = ESX_Model(input_dim=len(features),share_dim=12,base_dim=12)
  est7 = EUEN(input_dim=len(features), hc_dim=64, hu_dim=64, is_self=False)

  loss1 = partial(tarnet_loss)
  loss2 = partial(cfrnet_loss)
  loss3 = partial(dragonnet_loss, alpha=1.0, beta=1.0,tarreg=True)
  loss4 = partial(dragon_loss, alpha=1) 
