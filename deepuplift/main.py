...
from models.ganite import GANITE, ganite_loss  # 添加GANITE模型导入
...
if __name__ == "__main__":
  ...
  est9 = EEUEN(input_dim=len(features), hc_dim=64, hu_dim=64, he_dim=64, 
               is_self=True)
  est10 = GANITE(input_dim=len(features), h_dim=64, is_self=True)  # 添加GANITE模型实例化
  ...
  loss10 = partial(ganite_loss, alpha=1.0)  # 添加GANITE损失函数


  # 选择要测试的模型
  model,loss_f = est10,loss10  # 这里选择GANITE模型进行测试
